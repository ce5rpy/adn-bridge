/*
 * YSF FICH codec, DGID rewrite, and activation burst.
 *
...
static void fich_set_dt(unsigned char dt);
static void fich_set_fi(unsigned char fi);
static void fich_set_cs(unsigned char cs);
static void fich_set_cm(unsigned char cm);
static void fich_set_fn(unsigned char fn);
...
static void generate_voice_payload(unsigned char *payload, int frame_num)
{
    for (int i = 0; i < 100; i++) {
...
static const uint8_t YSF_SYNC_WORD[5] = {0xD4U, 0x71U, 0xC9U, 0x63U, 0x4DU};

static unsigned int ysf_fich_guess_rx_offset(const uint8_t *frame155)
{
    if (memcmp(frame155 + 35, YSF_SYNC_WORD, 5) == 0)
        return YSF_FICH_OFFSET_RX;
    return YSF_FICH_OFFSET_NET;
}

static void ysf_fich_decode_at(const uint8_t *frame155, unsigned int off,
                               uint8_t *fi, uint8_t *fn, uint8_t *ft,
                               uint8_t *cm, uint8_t *dt)
{
    fich_decode(frame155 + off);
    *fi = (m_fich[0U] >> 6) & 0x03U;
    *fn = (m_fich[1U] >> 3) & 0x07U;
    *ft = m_fich[1U] & 0x07U;
    *cm = (m_fich[0U] >> 2) & 0x03U;
    *dt = m_fich[2U] & 0x03U;
}
                           ...
                           uint8_t *ft, uint8_t *cm, uint8_t *dt)
{
    ...
    if (!frame155 || !fi || !fn || !ft || !cm || !dt)
        return -1;

    off = ysf_fich_guess_rx_offset(frame155);
    ysf_fich_decode_at(frame155, off, fi, fn, ft, cm, dt);
    if (*fi <= 3U)
        return 0;

    alt = (off == YSF_FICH_OFFSET_RX) ? YSF_FICH_OFFSET_NET : YSF_FICH_OFFSET_RX;
    ysf_fich_decode_at(frame155, alt, fi, fn, ft, cm, dt);
    return 0;
}
...
/* DT lives in bits 1-0 of m_fich[2] (CYSFFICH); dgidcon wrongly used bits 7-6. */
static void fich_set_dt(unsigned char dt)  { m_fich[2U] &= 0xFCU; m_fich[2U] |= dt & 0x03U; }
static void fich_set_fi(unsigned char fi)  { m_fich[0U] &= 0x3FU; m_fich[0U] |= (fi << 6) & 0xC0U; }
static void fich_set_cs(unsigned char cs)  { m_fich[0U] &= 0xCFU; m_fich[0U] |= (cs << 4) & 0x30U; }
static void fich_set_cm(unsigned char cm)  { m_fich[0U] &= 0xF3U; m_fich[0U] |= (cm << 2) & 0x0CU; }
static void fich_set_fn(unsigned char fn)  { m_fich[1U] &= 0xC7U; m_fich[1U] |= (fn << 3) & 0x38U; }
